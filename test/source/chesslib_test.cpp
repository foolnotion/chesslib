...
static constexpr bool is_sliding_v =
    ((P == piece::bishop) || (P == piece::rook) || (P == piece::queen));
...
auto print(board const& b, auto&& pred, char const* marker, fmt::color color)
{
    ...
    for (auto i = n - 1; i >= 0; --i) {
        for (auto j = 0; j < n; ++j) {
            ...
            auto const* c =
                piece_symbols[b.color_at(s) == color::white ? 0 : 1][p];
        ...
        if (p == piece::pawn) {
            ...
            if (tgt == square::g1 || tgt == square::g8) {
                ...
                throw std::runtime_error(
                    "castling flag set but target is inconsistent");
            }
            out.push_back(str);
        } else {
            str.push_back(piece_letters[0][me::enum_integer(p)]);
            auto const& moves_to_tgt =
                map.find({static_cast<u8>(p), tgt})->second;
            if (moves_to_tgt.size() > 1) {
                if (std::ranges::count_if(
                    ...
                    > 1)
                {
                    str.push_back(ranks[static_cast<size_t>(
                        coord::rank(m.source_square))]);
                } else {
                    str.push_back(files[static_cast<size_t>(
                        coord::file(m.source_square))]);
                }
            }
            if (m.capture) {
                str.push_back('x');
...
TEST_CASE("move generator", "[library]")
{
    auto check_test_cases = [](auto const& cases, auto piece)
    {
        for (auto fen : cases) {
            ...
            for (auto const& m : moves) {
                ...
                squares.insert({static_cast<u8>(m.target_square),
                                static_cast<u8>(m.source_square)});
            }
    ...
    SECTION("pawn moves")
    {
        constexpr std::array cases = {"8/k7/8/8/3pP3/8/8/K7 b - e3 0 1"};
    ...
    SECTION("rook moves")
    {
        constexpr std::array cases = {"8/k7/8/3R4/8/8/8/K7 w - - 0 1",
                                      "8/4k3/8/8/8/8/r6r/R3K2R w KQ - 0 1"};
    ...
    SECTION("king moves")
    {
        constexpr std::array cases = {"k7/8/8/8/8/8/8/4K2R w K - 0 1",
                                      "8/8/8/3K4/8/4k3/8/8 w - - 0 1",
                                      "8/8/8/3K4/8/4k3/8/8 b - - 0 1",
                                      "8/4k3/8/8/8/8/r6r/R3K2R w KQ - 0 1"};
...
TEST_CASE("attacked squares", "[library]")
{
    ...
    for (auto const* fen : test_cases) {
        ...
        helpers::print(board,
                       attacked,
                       "▢",
                       board.side() == side_to_move::white ? fmt::color::red
                                                           : fmt::color::blue);
        fmt::print("\n");
    }
}
...
TEST_CASE("king in check", "[library]")
{
    ...
    for (auto const* fen : test_cases) {
        ...
        fmt::print("king positions - white: {}, black: {}\n",
                   me::enum_name(b.state().white_king),
                   me::enum_name(b.state().black_king));
...
TEST_CASE("rampart", "[library]")
{
    ...
    auto check_case = [&](auto t)
    {
        ...
        auto tmp = b;
        auto move_invalid = [&](auto const& m)
        { return move_maker {b, m}.check(); };
        moves.erase(std::remove_if(moves.begin(), moves.end(), move_invalid),
                    moves.end());
        ASSERT(tmp == b);

        auto computed_moves = helpers::format_moves(b, moves);
        std::ranges::sort(computed_moves);
        auto color = expected_moves == computed_moves ? fmt::color::green
                                                      : fmt::color::red;
...
TEST_CASE("zobrist", "[library]")
{
    ...
    auto check_move = [&](auto const m)
    {
        ...
        auto h1 = hasher(b);
        h1 ^= zobrist::hasher::piece(
            i, j, src);  // taking the piece from its source square
        h1 ^= zobrist::hasher::piece(
            i, j, tgt);  // putting the piece on its destination square
        ...
        INFO(
            "Check end state (move made and undone, 2xincremental hash update");
        REQUIRE(h1 == hasher(b));
        REQUIRE(hasher(b) == zobrist::hasher::recompute(b));
    };
    ...
    SECTION("a2a4")
    {
        move m {.source_square = square::a2,
    ...
    // For all remaining sections: verify that after make+undo the
    // full-recomputed hash is identical to the hash before the move.  This
    // covers all the state that hasher::operator() inspects: piece placement,
    // castling rights, and the en-passant file.  make() toggles side-to-move
    // and undo() restores it, so the hash after undo() matches the hash before
    // make().
    SECTION("hash round-trip")
    {
        ...
        {
            // white d5-pawn can take e.p. on e6; black e5-pawn just
            // double-pushed
            check_round_trip(
    ...
    {
        // All 20 positions reachable in one move from the starting position
        // must hash distinctly (with make() not toggling side they share the
        // same side flag, so any collision would be a genuine piece-placement
        // collision).
        board start;
        zobrist::hasher h {};
...
{
    // For every legal move in a position, make then undo must restore the board
    // exactly.
...
TEST_CASE("fen errors", "[library]")
{
    ...
    {
        auto result =
            read("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR x KQkq - 0 1");
        REQUIRE(!result);
    ...
    {
        auto result =
            read("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - abc 1");
        REQUIRE(!result);
    ...
    {
        auto result =
            read("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 abc");
        REQUIRE(!result);
    ...
    {
        auto result =
            read("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq z9 0 1");
        REQUIRE(!result);
    ...
    {
        auto result =
            read("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -");
        REQUIRE(!result);
...
TEST_CASE("checkmate and stalemate", "[library]")
{
    ...
    SECTION("stalemate")
    {
        // Black king a8, white queen c7, white king b6 — black to move,
        // stalemate
...
TEST_CASE("uci", "[library]")
{
    SECTION("to_string")
    {
        move m {.source_square = square::e2,
        ...
        // Promotion to queen
        move q {.source_square = square::a7,
                .target_square = square::a8,
        ...
        // Knight promotion
        move n {.source_square = square::h7,
                .target_square = square::h8,
    ...
    {
        board b;
        REQUIRE(!uci::from_string(b, "e2e5")
                     .has_value());  // pawn can't jump 3 squares
...
TEST_CASE("san", "[library]")
{
    ...
    {
        // Bxf7+ after 1.e4 e5 2.Bc4: bishop captures on f7, checking the king
        // on e8
        board b {
    ...
    SECTION("to_string - checkmate")
    {
        // Scholar's mate: Qxf7#
        board b {
            "r1bqkb1r/pppp1ppp/2n2n2/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR w KQkq - 4 "
            "4"};
    ...
    SECTION("from_string - errors")
    {
        ...
        REQUIRE(san::from_string(b, "e9").error()
                == san::error::invalid_syntax);
        REQUIRE(san::from_string(b, "e5").error()
                == san::error::no_matching_move);
        REQUIRE(san::from_string(b, "Rd1").error()
                == san::error::no_matching_move);
    }
    ...
    SECTION("from_string - full square disambiguation")
    {
        ...
        // (simpler: just verify the general round-trip works for all legal
        // moves)
        for (auto const& m : legal_moves(b)) {
            auto const s = san::to_string(b, m);
            auto const result = san::from_string(b, s);
    ...
    SECTION("san_replay - rejects illegal pinned move with single candidate")
    {
        board b {"4r1k1/8/8/8/8/8/4R3/4K3 w - - 0 1"};
        san::replayer replay {b};

        auto result = replay.play("Rd2");

        REQUIRE(!result.has_value());
        REQUIRE(result.error() == san::error::no_matching_move);
        REQUIRE(b.export_fen() == "4r1k1/8/8/8/8/8/4R3/4K3 w - - 0 1");
    }
...
TEST_CASE("move codec", "[library]")
{
    ...
    SECTION("quiet move")
    {
        roundtrip(move {.source_square = square::e2,
                        .target_square = square::e3,
                        .promotion = 0,
                        .capture = 0,
                        .double_pawn = 0,
                        .enpassant = 0,
    ...
    SECTION("capture")
    {
        roundtrip(move {.source_square = square::d4,
    ...
    SECTION("en passant")
    {
        roundtrip(move {.source_square = square::e5,
                        .target_square = square::d6,
                        .promotion = 0,
                        .capture = 1,
                        .double_pawn = 0,
                        .enpassant = 1,
    ...
    {
        roundtrip(move {.source_square = square::e1,
    ...
    SECTION("promotions (quiet)")
    {
        ...
        {
            roundtrip(move {.source_square = square::a7,
                            .target_square = square::a8,
