...
struct move_generator {
    auto moves(move_list& m) const {
        ...
        for (u8 i = 0; i < detail_0x88::sz; ++i) {
            ...
            if ((side && c == color::black) || (!side && c == color::white)) {
                continue;
            }
            ...
            switch(p) {
                ...
                case piece::bishop: {
                    for (auto o : board::bishop_offsets) {
                ...
                case piece::queen: {
                    for (auto o : board::queen_offsets) {
                        for (u8 j = i + o; coord::valid(j) ; j += o) {
                            if (pieces[j] != piece::none) { break; }
                            auto [q, d] = board[j];
                            if (c == d) { continue; }
                ...
                case piece::king: {
                    ...
                    for (auto o : board::king_offsets) {
                        u8 j = i + o;
                        auto [q, d] = board[j];
                        if (coord::valid(j) && (q == piece::none || d != c)) {
